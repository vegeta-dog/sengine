...
void indexBuilder::builder::run() {
  while (true) {
    try {
      ...
      log->debug(__LINE__, "preprocess ok! ");
      ...
      for (auto x : inv_map) {
        ...
        t = new boost::thread(&worker, msg_obj, page_id, x.first,
                              get_inv_index_path(x.first, this->log, tmp_conn),
        ...
        if (work_threads.size() == 5) {
          for (auto tt : work_threads) tt->join();

          for (auto tt : work_threads) free(tt);  // delete tt
          work_threads.clear();
        }
      }
      // 不够5个线程时，直接执行
      ...
      for (auto tt : work_threads) free(tt);
      work_threads.clear();
...
void indexBuilder::preprocess(
  ...
  for (const auto &ar : arr) {
    for (boost::json::value item : ar) {
      ...
      std::cout << "item_str = " << str << std::endl;
...
void indexBuilder::worker(
  ...
  if (path != "-1") {
    ...
    if (existsed_list.page_set.count(id)) existsed_list.page_set.erase(id);
  ...
  // 获取document的全部内容
  for (auto &x : document_arr) document += (bj::value_to<std::string>(x));

  // 获取title字符串
  for (auto &x : title_arr) title += (bj::value_to<std::string>(x));
...
std::string indexBuilder::gen_invIndex_filepath(const int &id) {
  ...
  std::cout << "string id = " << boost::lexical_cast<std::string>(id)
            << std::endl;
  return indexBuilder::invIndex_file_base_path +
         boost::lexical_cast<std::string>(id) + '_' +
