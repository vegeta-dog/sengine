...
class ConstBuffer {
 public:
  ...
  std::string se_toString() const {
    if (_size == 0) return _data ? "[empty_string]" : "[null_pointer]";
    std::ostringstream oss;
    auto getChar = [&oss](const unsigned char c) { oss << c; };
		const auto* beg = static_cast<const unsigned char*> (_data);
		std::for_each(beg, beg + _size, getChar);
  ...
  std::string toString() const {
    ...
    // const auto* beg = static_cast<const std::string*>(_data);
    const auto* beg = static_cast<const unsigned char*>(_data);
    // const std::string* x = static_cast<const std::string*>(_data);
    std::for_each(beg, beg + _size, printChar);
    // std::cout << "*x===== " << *x << std::endl;
    // std::string ret = *x;

    // std::cout << "begin ! " << ret << std::endl;
